...
class Parse:
    async def get_page_data(self, session, link, group):
        exam_url = 'https://www.sut.ru/studentu/raspisanie/raspisanie-ekzamenacionnoy-sessii'
        headers = {
        ...
        async with session.get(url=link, headers=headers) as response:
            ...
            for i in range(1, len(html)):
                try:
                    ...
                    type = line[3].text.strip()
                    name = line[5].text.strip()
                    try:
                        teachers = line[7].text.strip().split('; ')
